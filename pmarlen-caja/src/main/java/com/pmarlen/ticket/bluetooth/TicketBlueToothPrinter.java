            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int i = 0; i < numIter; i++) {
                    ...
                    if (expandDetail) {
						...
						//${CODIGO } ${PRODUCTO}
						//   ${CANT} *  ${PRECIO}  ${IMP}
						staticVars.put("${CANT}"    , alignTextRigth(df_6.format(pvdList.get(i).getC()),3));
						String nombre = prod.getNombre();
                        //staticVars.put("${PRODUCTO}", alignTextLeft(prod.getNombre(),25));
						if(nombre.length()>28){
							nombre = nombre.substring(28);
						...
						staticVars.put("${CODIGO}", alignTextRigth(pvdList.get(i).getCb(),14));                        
						
                        importe  = pvdList.get(i).getC() * pvdList.get(i).getP();
                        sum_importe += importe;
                        sum_desc += desc;

                        staticVars.put("${PRECIO}" , alignTextRigth(dfs4_2.format(pvdList.get(i).getP()),7));
